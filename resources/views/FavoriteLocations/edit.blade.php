...
<body class="bg-gray-100 font-sans antialiased text-gray-900 min-h-screen">
    <div class="max-w-xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-md transition duration-300 hover:shadow-xl">
        ...
        <form action="{{ route('favorite-locations.update', $favoriteLocation->id) }}" method="POST" autocomplete="off">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input 
                    ...
                    name="label" 
                    id="label" 
                    value="{{ old('label', $favoriteLocation->label) }}"
            ...
            <div class="mb-6">
                ...
                <textarea 
                    ...
                    required
                >{{ old('alamat', $favoriteLocation->alamat) }}</textarea>
                @error('alamat')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-end space-x-3 border-t border-gray-100 pt-4">
                <a href="{{ route('favorite-locations.index') }}" 
                   class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-100 transition duration-150">
                    Kembali ke Daftar
                </a>
