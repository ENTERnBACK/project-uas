...
<body class="bg-gray-100 font-sans antialiased text-gray-900 min-h-screen">
    ...
    <div class="max-w-xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-md transition duration-300 hover:shadow-xl">
        ...
        <!-- Form Edit Data -->
        <!-- Action diarahkan ke route update dengan membawa ID data yang sedang diedit -->
        <form action="{{ route('favorite-locations.update', $location->id) }}" method="POST">
            @csrf
            <!-- Laravel mewajibkan method PUT atau PATCH untuk proses update data -->
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input 
                    ...
                    required
                >
                <!-- Menampilkan pesan error validasi untuk label -->
                @error('label')
            ...
            <div class="mb-6">
                ...
                <!-- Menampilkan pesan error validasi untuk alamat -->
                @error('alamat')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
